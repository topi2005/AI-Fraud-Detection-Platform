...
# VPC with public + private subnets, NAT gateway, and all security groups
...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "${var.prefix}-public-${count.index + 1}", Tier = "public" }
}

# ── Private subnets (ECS, RDS, MSK, ElastiCache) ─────────────────────────────
resource "aws_subnet" "private" {
  count             = length(var.availability_zones)
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, count.index + 10)
  availability_zone = var.availability_zones[count.index]
  tags = { Name = "${var.prefix}-private-${count.index + 1}", Tier = "private" }
}
...
# ── NAT Gateway (one per AZ for prod HA; one total for dev/cost savings) ─────
